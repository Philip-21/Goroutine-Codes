...
import (
	...
	"github.com/fatih/color" //displays colorful outputs in terminal
)
...
func makePizza(pizzaNumber int) *PizzaOrder {
	pizzaNumber++ //increment by 1 cause th integer was initially 0

	//odering the pizza
	if pizzaNumber <= NumberOfPizzas {
		delay := rand.Intn(5) + 1 //random int btwn 1 & 5
		fmt.Printf("Received order #%d!\n", pizzaNumber)

		//making the Pizza
		rnd := rand.Intn(12) + 1 //generating  a random no btwn 1 and 12
		msg := ""
		success := false
	...
	return &PizzaOrder{
		pizzaNumber: pizzaNumber, //refrence to a pizza order with pizza number of 11 cause the no of pizas is set to 10 from the const
	}
}
...
func pizzeria(pizzaMaker *Producer) {
	...
	var i = 0
	//run forevr until  we receive a quit notification
	...
	for {
		...
		if currentPizza != nil {
			...
			select { //select statement are useful for chanels synonymous to switch statement
...
func main() {
	...
	// create a producer
	pizzajob := &Producer{
		data: make(chan PizzaOrder), //putting the pizza order in a channel to be executed by the go routine
